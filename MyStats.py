...
class MyStat:
    ...
    def show_period_hist(self):
        periods = [wave.T for wave in self.waves]
        ...
        # dens = lognorm.pdf(sorted(periods), s=height_deviation, scale=height_mean)
        # pirson(norm_periods, dens)
        # print(scipy.stats.chisquare(norm_periods, dens))

        # гистограмма периода
        plt.hist(sorted(set(periods)), edgecolor='black', weights=norm_periods, bins=math.ceil(math.sqrt(len(periods))))
        plt.title("Гистограмма относительных частот периода волн")
        plt.xlabel('T, минуты')
        plt.ylabel('Частота, []')
        plt.plot(sorted(list(set(periods))), dens, linewidth=2, color='r')
        # plt.plot(sorted(periods), dens)
        plt.show()